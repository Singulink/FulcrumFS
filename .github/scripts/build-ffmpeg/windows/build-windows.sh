...
# Writes ffmpeg-windows-x64.zip (ffmpeg + ffprobe at the zip root) into $FFMPEG_OUTPUT_DIR.
...
# Build for Windows x64 from Linux x64 - result goes into '~/Clones/ffmpeg-windows-build-helpers/sandbox/win64/ffmpeg_git_with_fdk_aac' (ffmpeg.exe & ffprobe.exe are self contained)
mkdir -p ~/Clones
cd ~/Clones
