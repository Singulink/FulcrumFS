...
partial class FileRepo
{
    ...
    private async ValueTask EnsureInitializedAsync(bool forceHealthCheck, CancellationToken cancellationToken)
    {
        ...
        using (await _stateSync.LockAsync(MaxAccessWaitOrRetryTime, cancellationToken))
        {
            if (_lockStream is null || forceHealthCheck || Stopwatch.GetElapsedTime(_lastSuccessfulHealthCheck) >= HealthCheckInterval)
            {
                ...
                while (true)
                {
                    ...
                    if (!elc.TryRun(InitializeCore) && Stopwatch.GetElapsedTime(initStartTimestamp) > MaxAccessWaitOrRetryTime)
