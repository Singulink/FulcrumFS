...
/// <content>
/// Contains a test-only instrumentation seam used to deterministically reproduce crashes at well-defined points in the multi-step delete/rebase flow. The
/// hook is <see langword="internal"/> and is never set in production use; it is exercised only by the test assembly via <c>InternalsVisibleTo</c>. Because the
/// delete/rebase flow is strictly forward-only (every step persists a marker and never compensates on failure), throwing from the hook at a checkpoint leaves
/// exactly the on-disk state a real crash would, which lets recovery (cleaner rollforward, or the add/delete optimistic rollforward) be tested faithfully.
/// </content>
partial class FileRepo
{
    ...
    /// use. A test may throw from the hook to simulate a crash at a precise point, or use it purely to observe step ordering.
    /// </summary>
