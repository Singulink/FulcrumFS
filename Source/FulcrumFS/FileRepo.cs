...
public sealed partial class FileRepo : IDisposable
{
    ...
    /// <summary>
    /// Gets a value indicating whether the repository logs information and errors to indeterminate and delete marker files.
    /// </summary>
    /// <remarks>
    /// See <see cref="FileRepoOptions.LogToMarkerFiles"/> for more information on this setting.
    /// </remarks>
    public bool LogToMarkerFiles { get; }
    ...
    public FileRepo(FileRepoOptions options)
    {
        ...
        LogToMarkerFiles = options.LogToMarkerFiles;
    ...
    private IAbsoluteDirectoryPath GetDataFileGroupDirectory(FileId fileId)
    {
        if (fileId.String.Length is not 32)
            throw new ArgumentException("File ID string must be 32 characters long.", nameof(fileId));
