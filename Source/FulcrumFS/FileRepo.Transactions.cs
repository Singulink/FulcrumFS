...
partial class FileRepo
{
    ...
    internal async Task TxnDeleteAsync(FileId fileId)
    {
        ...
        using (await _fileSync.LockAsync((fileId, null)).ConfigureAwait(false))
        {
            ...
            await LogToMarkerAsync(indeterminateMarker, "TRANSACTION PENDING DELETE", message, ignoreErrors: false).ConfigureAwait(false);
        }
    }
    ...
    private async Task DeleteIndeterminateMarkerAsync(FileId fileId)
    {
        ...
        using (await _fileSync.LockAsync((fileId, null)).ConfigureAwait(false))
        {
            // Delete the indeterminate marker
            ...
            catch (IOException ex) when (ex.GetType() != typeof(FileNotFoundException))
            {
                ...
                await LogToMarkerAsync(indeterminateMarker, "DELETE INDETERMINATE MARKER FAILED", ex.ToString(), ignoreErrors: true).ConfigureAwait(false);
            }
        }
    }
}
